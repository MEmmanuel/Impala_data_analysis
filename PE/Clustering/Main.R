...
### Ce script permet de clusteriser les métiers, de les représenter graphiquement, et de préparer
### un fichier d'entrée pour le tf-idf 
...
file_path = "~/PycharmProjects/ImpalaPoleEmploi2/Impala/PE Clustering et tf-idf/Clustering"
setwd(file_path)
...
df = tsne(df_proc, perplexity = 40, seed = 1)
df_clusters = clustering(df, method="HC", K = 20)
plot_tsne(df_clusters)

# Clustering des métiers : pas la peine de recalculer le t-SNE à chaque fois 
# et on peut choisir le nombre de clusters K (en fait on obtiendra seulement une valeur
